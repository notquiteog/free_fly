  ...
  mod.events:on("game.ready", function()
    ...
    OC.__freeFlyTick = function(ow, dt)
      ...
      if voxelOn then
        ...
        -- Which rungs need the placed camera.
        --
        -- On Gen 1 only the 75-degree orbit does: the other rungs get their
        -- height indirectly, by moving the engine's ground-plane camera
        -- point back by camLift (below), and at 75 the camera is nearly
        -- overhead so moving the ground point buys nothing.
        --
        -- On Gold that indirect route does not exist. The camera follow is
        -- skipped there on purpose -- Gold's World:update drives its own
        -- camera every frame and would overwrite anything set here -- so
        -- with the placed seam gated to 75, NO rung got any lift at all:
        -- the rider climbed toward a camera that never backed off and the
        -- diorama showed a very large trainer standing on the grass. Every
        -- orbit rung takes the placed camera there.
        local orbit = not ((state.voxelStateRef.isFirstPerson
                            and state.voxelStateRef.isFirstPerson(rung))
                           or (state.voxelStateRef.isThirdPerson
                               and state.voxelStateRef.isThirdPerson(rung)))
        state.placeWanted = orbit
          and (deg == 75 or (Sky.goldWorld(ow) and deg > 0))
      ...
      if state.placeWanted and vsRef and V3
        ...
        local ok = pcall(function()
          -- Sky.viewSize, not Game.renderer: Gold has no Renderer
          -- singleton, so the direct read answered nil here and the
          -- whole pcall below failed silently -- the camera was never
          -- placed on a Gen 2 boot. The helper is this mod's own and
          -- already prefers the World's viewW/viewH there.
