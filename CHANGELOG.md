...
Flight itself always worked on Gold, Silver and Crystal -- takeoff is
accepted, `isFlying` goes true, the altitude ramps to cruise. The VIEW did
not: at the default FULL rung a Crystal boot showed the rider climbing
toward a camera that never backed off, so the diorama filled with a very
large trainer standing on the grass.

The mod raises the camera two different ways, and which one it uses is a
property of the rung. Rungs 15/35/50 get their height indirectly, by moving
the engine's ground-plane camera point back by `camLift`; only the
75-degree orbit needs the scene's placed-camera seam, because there the
camera is nearly overhead and moving the ground point buys nothing.

On Gold the indirect route does not exist. The camera follow is skipped
there on purpose -- Gold's `World:update` drives its own camera every frame
and would overwrite anything set from here -- so with the placed seam gated
to `deg == 75`, **no** rung got any lift at all.
...
singleton. Inside the block's own `pcall` that raised, the pcall swallowed
it, and the handler then set `state.placeWanted = false` for the rest of the
flight -- so the failure was not merely silent, it was sticky.

- the placed camera now takes its view size from `Sky.viewSize`, this mod's
  own helper, which already prefers the World's `viewW`/`viewH` on Gold;
- on a Gold world every orbit rung takes the placed camera. Gen 1 still
  reserves it for 75, where the ground-plane route is the better answer.

Measured on a real Crystal boot, mid-flight at FULL: `Voxel3D.camera` was
`nil` before and is a table after.

`tests/free_fly_gen2_camera_test.lua` covers both halves and was
mutation-checked: reverting the view-size read fails 2 checks, re-gating
`placeWanted` to 75 fails 1. The upstream suite is unchanged by this fork --
4/11 passing from the engine root, byte-identical to upstream 1.8.0 (the
other 7 need the engine's ROM-generated data, which a source checkout has
no way to produce).
